...
use strato_core::event::{Event, MouseButton, MouseEvent, KeyboardEvent, KeyCode, Modifiers, WindowEvent};
use std::sync::Arc;
use std::rc::Rc;
use winit::window::Window;
use crate::Application;
use strato_renderer::Backend;
use strato_renderer::backend::WgpuBackend;
            ...
            match event {
                WinitEvent::WindowEvent { event, .. } => {
                    match event {
                        ...
                        _ => {
                            if let Some(strato_event) = convert_window_event(event, cursor_position, scale_factor) {
                                handler(strato_event);
                            }
                        }
                    }
            ...
            match event {
                ...
                WinitEvent::WindowEvent { event, .. } => {
                    match event {
                        ...
                        _ => {
                            if let Some(strato_event) = convert_window_event(event, state.cursor_position, state.scale_factor) {
                                handler(strato_event);
                            }
                        }
                    }
                    ...
                    match event {
                        ...
                        WindowEvent::CursorMoved { position, device_id, .. } => {
                            ...
                            if let Some(strato_event) = convert_window_event(
                                ...
                                // println!("EventLoop: CursorMoved to {:?} (Logical), Scale: {}", strato_event, state.scale_factor);
                        ...
                        WindowEvent::Resized(physical_size) => {
                            ...
                            let event = strato_core::event::Event::Window(strato_core::event::WindowEvent::Resize {
                        ...
                        WindowEvent::CloseRequested => {
                            let event = strato_core::event::Event::Window(strato_core::event::WindowEvent::Close);
                            if let Some(app) = &mut state.app {
                                app.handle_event(event.clone());
                            }
                            handler(event);
                        ...
                        _ => {
                            if let Some(strato_event) = convert_window_event(event, state.cursor_position, state.scale_factor) {
                                // println!("EventLoop: Event {:?}", strato_event);
                                if let Some(app) = &mut state.app {
                                    app.handle_event(strato_event.clone());
                                }
                                handler(strato_event);
...
/// Convert winit key code to StratoUI key code
#[cfg(not(target_arch = "wasm32"))]
