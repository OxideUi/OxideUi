...
use strato_core::types::{Rect, Point, Transform, Color};
...
impl ScrollView {
    ...
    fn update_scrollbar_rect(&self, content_height: f32, viewport_height: f32, offset_y: f32, bounds: Rect) {
        if content_height > viewport_height {
            ...
            self.scrollbar_rect.set(Rect::new(scrollbar_x, scrollbar_y, scrollbar_width, thumb_height));
        } else {
...
impl Widget for ScrollView {
    ...
    fn render(&self, batch: &mut RenderBatch, layout: Layout) {
        ...
        self.update_scrollbar_rect(self.content_size.height, layout.size.height, self.offset.y, bounds);
        ...
        if scrollbar.width > 0.0 {
            ...
            batch.add_rect(
                scrollbar,
                if self.is_dragging { Color::rgba(0.4, 0.4, 0.4, 0.8) } else { Color::rgba(0.5, 0.5, 0.5, 0.5) },
                Transform::identity(),
            );
        }
    }
                ...
                if self.is_dragging {
                    ...
                    if content_h > viewport_h {
                        ...
                        if track_range > 0.0 {
                            ...
                            self.offset.y = (self.offset_start_y + offset_delta).clamp(0.0, max_offset);
                        }
                    }
