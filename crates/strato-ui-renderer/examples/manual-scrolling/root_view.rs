...
use strato_ui::elements::new_scrollable::AxisConfiguration;
use strato_ui::elements::new_scrollable::ClippedAxisConfiguration;
use strato_ui::elements::new_scrollable::DualAxisConfig;
use strato_ui::elements::new_scrollable::NewScrollableElement;
use strato_ui::elements::new_scrollable::ScrollableAppearance;
use strato_ui::elements::new_scrollable::ScrollableAxis;
use strato_ui::elements::Axis;

use strato_ui::elements::ChildView;
use strato_ui::elements::ClippedScrollStateHandle;
use strato_ui::elements::NewScrollable;
use strato_ui::elements::Point;
use strato_ui::elements::ScrollData;
use strato_ui::elements::ScrollStateHandle;
use strato_ui::keymap::FixedBinding;
use strato_ui::units::Pixels;
use strato_ui::AppContext;
use strato_ui::TypedActionView;
use strato_ui::ViewHandle;
use strato_ui::{
    elements::{ConstrainedBox, ParentElement, Rect, ScrollbarWidth, Stack},
    Element, Entity, View, ViewContext,
};

use strato_ui::color::ColorU;

pub fn init(ctx: &mut AppContext) {
    use strato_ui::keymap::macros::*;
...
impl Element for ScrollableElement {
    fn layout(
        &mut self,
        constraint: strato_ui::SizeConstraint,
        _: &mut strato_ui::LayoutContext,
        _: &AppContext,
    ) -> Vector2F {
        let size = vec2f(
    ...
    fn after_layout(&mut self, _: &mut strato_ui::AfterLayoutContext, _: &AppContext) {}

    fn paint(&mut self, origin: Vector2F, ctx: &mut strato_ui::PaintContext, _app: &AppContext) {
    ...
    fn dispatch_event(
        &mut self,
        _: &strato_ui::event::DispatchedEvent,
        _: &mut strato_ui::EventContext,
        _: &AppContext,
    ) -> bool {
        false
    }
