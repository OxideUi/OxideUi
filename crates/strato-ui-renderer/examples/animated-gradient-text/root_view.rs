...
use strato_ui::elements::shimmering_text::{
    ShimmerConfig, ShimmeringTextElement, ShimmeringTextStateHandle,
};
use strato_ui::elements::{Align, ConstrainedBox, ParentElement, Rect, Stack};
use strato_ui::fonts::FamilyId;
use strato_ui::SingletonEntity as _;
use strato_ui::{AppContext, Element, Entity, TypedActionView, View, ViewContext};
...
impl RootView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let font_family = strato_ui::fonts::Cache::handle(ctx).update(ctx, |cache, _| {
            cache
