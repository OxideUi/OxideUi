//! Core widget traits and types for OxideUI
//!
...
pub trait Widget: Debug + Send + Sync {
    ...
    /// Handle an event
    fn handle_event(&mut self, event: &Event, context: &mut WidgetContext) -> OxideResult<bool>;

    /// Update the widget's state
    fn update(&mut self, context: &mut WidgetContext) -> OxideResult<()>;

    /// Calculate the widget's layout
    fn layout(&mut self, constraints: &LayoutConstraints, context: &mut WidgetContext) -> OxideResult<Size>;

    /// Render the widget
    fn render(&self, context: &WidgetContext) -> OxideResult<()>;
...
impl WidgetManager {
    ...
    /// Handle an event
    pub fn handle_event(&mut self, event: &Event) -> OxideResult<bool> {
    ...
    /// Layout all widgets
    pub fn layout(&mut self, constraints: &LayoutConstraints) -> OxideResult<()> {
        if let Some(root) = self.tree.root() {
            let id = root.id();
            if let (Some(widget), Some(context)) = (
                self.tree.get_widget_mut(id),
    ...
    /// Render all widgets
    pub fn render(&self) -> OxideResult<()> {
        for id in self.tree.widget_ids() {
            if let (Some(widget), Some(context)) = (
    ...
    pub fn set_focus(&mut self, id: Option<WidgetId>) -> OxideResult<()> {
...
mod tests {
    ...
    impl Widget for TestWidget {
        ...
        fn handle_event(&mut self, _event: &Event, _context: &mut WidgetContext) -> OxideResult<bool> {
            Ok(false)
        }

        fn update(&mut self, _context: &mut WidgetContext) -> OxideResult<()> {
            Ok(())
        }

        fn layout(&mut self, _constraints: &LayoutConstraints, _context: &mut WidgetContext) -> OxideResult<Size> {
            Ok(Size::new(100.0, 50.0))
        }

        fn render(&self, _context: &WidgetContext) -> OxideResult<()> {
            Ok(())
        }
