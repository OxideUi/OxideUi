...
impl ShaderManager {
    ...
    pub fn load_shader(
        ...
        // Cache the result
        self.shader_cache.write().insert(cache_key, compiled.clone());
    ...
    fn load_source(&self, path: &Path, stage: ShaderStage) -> Result<Arc<ShaderSource>> {
        ...
        // Cache the source
        self.source_cache.write().insert(path.to_path_buf(), source.clone());
    ...
    fn detect_shader_language(&self, path: &Path, content: &str) -> Result<ShaderLanguage> {
        ...
        if content.contains("@vertex") || content.contains("@fragment") || content.contains("@compute") {
        ...
        loop {
            ...
            for cap in include_re.captures_iter(&processed.clone()) {
                ...
                let include_content = fs::read_to_string(&full_path)
                    .with_context(|| format!("Failed to read include file: {}", full_path.display()))?;
    ...
    fn extract_dependencies(&self, content: &str, base_path: &Path) -> Result<(HashSet<PathBuf>, HashSet<PathBuf>)> {
        let include_re = Regex::new(r#"#include\s+"([^"]+)""#)?;
    ...
    fn update_dependency_graph(&self, path: &Path, dependencies: HashSet<PathBuf>) {
        ...
        // Update current node
        let node = graph.entry(path.to_path_buf()).or_insert_with(|| DependencyNode {
            path: path.to_path_buf(),
        ...
        node.last_modified = fs::metadata(path).ok()
            .and_then(|m| m.modified().ok())
            .unwrap_or(SystemTime::now());
        
        // Update dependent nodes
        for dep_path in dependencies {
            let dep_node = graph.entry(dep_path.clone()).or_insert_with(|| DependencyNode {
                path: dep_path.clone(),
                dependents: HashSet::new(),
                dependencies: HashSet::new(),
                last_modified: SystemTime::now(),
    ...
    fn compile_shader(&self, source: &ShaderSource, variant: ShaderVariant) -> Result<Arc<CompiledShader>> {
    ...
    fn is_shader_file(&self, path: &Path) -> bool {
        if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
            matches!(ext.to_lowercase().as_str(), "wgsl" | "glsl" | "hlsl" | "vert" | "frag" | "comp")
        } else {
            false
        }
    }
    ...
    fn invalidate_shader_cache(&self, path: &Path) {
        ...
        cache.retain(|_, compiled| {
            !dependents.contains(&compiled.source_hash)
        });
    ...
    pub fn add_include_directory(&self, path: impl AsRef<Path>) {
        self.include_directories.write().push(path.as_ref().to_path_buf());
    }
    ...
    pub fn get_cache_stats(&self) -> (usize, usize, usize) {
        ...
        (shader_cache_size, source_cache_size, preprocessor_cache_size)
    }
}
