...
impl Backend for WgpuBackend {
    ...
    fn resize(&mut self, width: u32, height: u32) {
        if let (Some(surface_mgr), Some(device_mgr), Some(buffer_mgr)) = (&mut self.surface_mgr, &self.device_mgr, &mut self.buffer_mgr) {
            ...
            let projection = glam::Mat4::orthographic_rh(0.0, logical_width as f32, logical_height as f32, 0.0, -1.0, 1.0);
            buffer_mgr.upload_projection(device_mgr.queue(), &projection.to_cols_array_2d());
        }
    }
    ...
    fn set_scale_factor(&mut self, scale_factor: f64) {
        ...
        if let (Some(surface_mgr), Some(device_mgr), Some(buffer_mgr)) = (&self.surface_mgr, &self.device_mgr, &mut self.buffer_mgr) {
            let width = surface_mgr.width();
            let height = surface_mgr.height();
    ...
    fn submit(&mut self, commands: &[RenderCommand]) -> Result<()> {
        ...
        for cmd in commands {
            match cmd {
                ...
                RenderCommand::DrawRect { rect, color, transform } => {
                    ...
                    let transform = transform.unwrap_or(oxide_core::types::Transform::identity());
        ...
        let mut encoder = device_mgr.device().create_command_encoder(&CommandEncoderDescriptor {
